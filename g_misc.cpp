...
void ThrowGib (edict_t *self, const char *gibname, int damage, int type, int frame, int skinnum)
{
	edict_t *gib;
	vec3_t	vd;
	vec3_t	origin;
	...
	PhysX_ConvertToPhysX(gib, PxConvexMeshGeometry(PhysX_CookMD2(gibname, frame)), type == GIB_ORGANIC ? 2.8f : 4.4f);

	if (type == GIB_ORGANIC)
		gib->takedamage = DAMAGE_BLOOD;
	gib->flags &= ~FL_NO_KNOCKBACK;
}

void ThrowHead (edict_t *self, const char *gibname, int damage, int type, int frame, int skinnum)
{
	vec3_t	vd;
	float	vscale;
	...
	PhysX_ConvertToPhysX(self, PxConvexMeshGeometry(PhysX_CookMD2(gibname, frame)), type == GIB_ORGANIC ? 2.8f : 4.4f);

	if (type == GIB_ORGANIC)
		self->takedamage = DAMAGE_BLOOD;
	self->flags &= ~FL_NO_KNOCKBACK;
}
...
void ThrowDebris (edict_t *self, const char *modelname, float speed, vec3_t origin)
{
	...
	PhysX_ConvertToPhysX(chunk, PxConvexMeshGeometry(PhysX_CookMD2(modelname)), 0.7);
}
