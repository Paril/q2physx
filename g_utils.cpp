...
edict_t *G_Find (edict_t *from, int fieldofs, const char *match)
{
	...
	for ( ; from < &g_edicts[globals.num_edicts] ; from++)
	{
		if (!from->inuse)
			continue;
		s = (allocated_string *) ((byte *)from + fieldofs);
		if (!s->length)
			continue;
		if (!Q_stricmp (s->data, match))
			return from;
	}
...
//
	...
	{
		gi.centerprintf (activator, "%s", ent->message.data);
		if (ent->noise_index)
...
//		gi.dprintf("tried to free special edict\n");
	...
	if (ed->physicsBody)
	{
		auto body = static_cast<PxActor *>(ed->physicsBody);
		scene->lockWrite();
