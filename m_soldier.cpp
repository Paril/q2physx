...
// check for gib
	if (self->health <= self->gib_health)
	{
		...
		ThrowGib (self, "models/monsters/soldier/tris.md2", damage, GIB_ORGANIC, 476, self->s.skinnum | 1);
		ThrowGib (self, "models/monsters/soldier/tris.md2", damage, GIB_ORGANIC, 478, self->s.skinnum | 1);
		ThrowGib (self, "models/monsters/soldier/tris.md2", damage, GIB_ORGANIC, 479, self->s.skinnum | 1);
		ThrowGib (self, "models/monsters/soldier/tris.md2", damage, GIB_ORGANIC, 480, self->s.skinnum | 1);
		ThrowGib (self, "models/monsters/soldier/tris.md2", damage, GIB_ORGANIC, 481, self->s.skinnum | 1);
		ThrowGib (self, "models/monsters/soldier/tris.md2", damage, GIB_ORGANIC, 482, self->s.skinnum | 1);
		ThrowGib (self, "models/monsters/soldier/tris.md2", damage, GIB_ORGANIC, 483, self->s.skinnum | 1);
		ThrowGib (self, "models/monsters/soldier/tris.md2", damage, GIB_ORGANIC, 484, self->s.skinnum | 1);
		ThrowGib (self, "models/monsters/soldier/tris.md2", damage, GIB_ORGANIC, 485, self->s.skinnum | 1);
		ThrowGib (self, "models/monsters/soldier/tris.md2", damage, GIB_ORGANIC, 486, self->s.skinnum | 1);
		ThrowGib (self, "models/monsters/soldier/tris.md2", damage, GIB_ORGANIC, 487, self->s.skinnum | 1);
		ThrowGib (self, "models/monsters/soldier/tris.md2", damage, GIB_ORGANIC, 488, self->s.skinnum | 1);
		ThrowGib (self, "models/monsters/soldier/tris.md2", damage, GIB_ORGANIC, 489, self->s.skinnum | 1);
		ThrowHead (self, "models/monsters/soldier/tris.md2", damage, GIB_ORGANIC, 477, self->s.skinnum | 1);
		self->deadflag = DEAD_DEAD;
		return;*/

		// helper function to spawn the given gib piece at the specified origin/angles
		auto spawnPiece = [](const char *gib, int frame, int skin, const auto &offset, const auto &angles, float density)
		{
			edict_t *p = G_Spawn();
			p->s.modelindex = gi.modelindex(gib);
			p->takedamage = DAMAGE_BLOOD;
			p->clipmask = MASK_SHOT;
			p->s.frame = frame;
			p->s.skinnum = skin;
			p->s.effects |= EF_GIB;
			VectorCopy(offset, p->s.origin);
			VectorCopy(angles, p->s.angles);
			gi.linkentity(p);

			PhysX_ConvertToPhysX(p, PxConvexMeshGeometry(PhysX_CookMD2(gib, frame)), density);

			// FIXME: dumb, fix this later
			scene->lockWrite();
			scene->removeActor(*(PxActor *)p->physicsBody);
			scene->unlockWrite();

			return p;
		};
		
		// spawn ragdoll pieces relative to the origin
		edict_t *pieces[] = {
			spawnPiece("models/monsters/soldier/tris.md2", 476, self->s.skinnum | 1, vec3_origin, PxVec3(0, 0, 0), 1.4),

			spawnPiece("models/monsters/soldier/tris.md2", 477, self->s.skinnum | 1, PxVec3(3, 0, 10), PxVec3(0, 0, 0), 1.4),

			spawnPiece("models/monsters/soldier/tris.md2", 478, self->s.skinnum | 1, PxVec3(2, 0, -12), PxVec3(0, 0, 0), 1.4),

			spawnPiece("models/monsters/soldier/tris.md2", 479, self->s.skinnum | 1, PxVec3(2, -(8 + 6), 0), PxVec3(0, 0, 0), 1.4),
			spawnPiece("models/monsters/soldier/tris.md2", 480, self->s.skinnum | 1, PxVec3(1, -(8 + 6 + 6 + 10), 0), PxVec3(0, 0, 0), 1.4),

			spawnPiece("models/monsters/soldier/tris.md2", 481, self->s.skinnum | 1, PxVec3(2, 8 + 5, 0), PxVec3(0, 0, 0), 1.4),
			spawnPiece("models/monsters/soldier/tris.md2", 482, self->s.skinnum | 1, PxVec3(2, 8 + 5 + 6 + 3 + 1, 0), PxVec3(0, 0, 0), 1.4),
			spawnPiece("models/monsters/soldier/tris.md2", 483, self->s.skinnum | 1, PxVec3(2, 8 + 5 + 6 + 3 + 6 + 1, 0), PxVec3(0, 0, 0), 1.4),
			
			spawnPiece("models/monsters/soldier/tris.md2", 484, self->s.skinnum | 1, PxVec3(2.75, -4, -(12 + 11.25)), PxVec3(0, 0, 0), 1.4),
			spawnPiece("models/monsters/soldier/tris.md2", 485, self->s.skinnum | 1, PxVec3(2.75, -4, -(12 + 26)), PxVec3(0, 0, 0), 1.4),
			spawnPiece("models/monsters/soldier/tris.md2", 486, self->s.skinnum | 1, PxVec3(5.75, -4.5, -(12 + 34.5)), PxVec3(0, 0, 0), 1.4),
			
			spawnPiece("models/monsters/soldier/tris.md2", 487, self->s.skinnum | 1, PxVec3(2.75, 3.5, -(12 + 12.25)), PxVec3(0, 0, 0), 1.4),
			spawnPiece("models/monsters/soldier/tris.md2", 488, self->s.skinnum | 1, PxVec3(2.75, 3.5, -(12 + 27)), PxVec3(0, 0, 0), 1.4),
			spawnPiece("models/monsters/soldier/tris.md2", 489, self->s.skinnum | 1, PxVec3(5.75, 4.0, -(12 + 35.5)), PxVec3(0, 0, 0), 1.4),
		};
		
		// position for ragdoll
		PxTransform finalCenter({ self->s.origin[0], self->s.origin[1], self->s.origin[2] + 30 }, PxIdentity);
		
		// helper function to create a ball-and-socket-esque joint that has a cone-esque
		// limited swing.
		auto createConeJoint = [pieces, &finalCenter](int a, int b, PxVec3 worldJointAnchor)
		{
			auto joint = PxD6JointCreate(*physics,
				(PxRigidActor *) pieces[a]->physicsBody,
				((PxRigidActor *) pieces[a]->physicsBody)->getGlobalPose().getInverse() * PxTransform(worldJointAnchor),
				(PxRigidActor *) pieces[b]->physicsBody,
				((PxRigidActor *) pieces[b]->physicsBody)->getGlobalPose().getInverse() * PxTransform(worldJointAnchor));
			joint->setConstraintFlag(PxConstraintFlag::eCOLLISION_ENABLED, false);
			joint->setTwistLimit(PxJointAngularLimitPair(-0.5, 0.5));
			joint->setMotion(PxD6Axis::eTWIST, PxD6Motion::eLIMITED);
			joint->setSwingLimit(PxJointLimitCone(0.5, 0.5));
			joint->setMotion(PxD6Axis::eSWING1, PxD6Motion::eLIMITED);
			joint->setMotion(PxD6Axis::eSWING2, PxD6Motion::eLIMITED);

			//edict_t *p = G_Spawn();
			//p->s.modelindex = gi.modelindex("models/objects/grenade2/tris.md2");
			//VectorAdd(worldJointAnchor, finalCenter.p, p->s.origin);
			//gi.linkentity(p);
		};
		
		createConeJoint(0, 1, PxVec3(2.2, 0, 7));
		createConeJoint(0, 2, PxVec3(3, 0, -7));
		createConeJoint(0, 3, PxVec3(2, -8, 0.5));
		createConeJoint(3, 4, PxVec3(2, -(8 + 12), 0.5));
		createConeJoint(0, 5, PxVec3(2, 8, 0.5));
		createConeJoint(5, 6, PxVec3(2, 8 + 12, 0.5));
		createConeJoint(6, 7, PxVec3(2, 8 + 18, 0.5));
		createConeJoint(0, 8, PxVec3(3.2, -3.3, -17));
		createConeJoint(8, 9, PxVec3(3.2, -4.2, -32));
		createConeJoint(9, 10, PxVec3(3.8, -4.0, -44));
		createConeJoint(0, 11, PxVec3(3.2, 2.8, -17));
		createConeJoint(11, 12, PxVec3(3.2, 3.7, -32));
		createConeJoint(12, 13, PxVec3(3.8, 3.7, -44));
		
		scene->lockWrite();

		// throw all of the pieces in an aggregate
		auto aggregate = physics->createAggregate(sizeof(pieces) / sizeof(pieces[0]), true);
		
		for (auto piece : pieces)
		{
			auto &actor = *(PxRigidBody *)piece->physicsBody;
			auto relative = actor.getGlobalPose();
			// FIXME: rotate the ragdoll to match self->s.angles[1]
			actor.setGlobalPose(finalCenter * relative);
			aggregate->addActor(actor);
			QuakeFromTransform(actor.getGlobalPose(), piece->s.origin, piece->s.angles);
		}

		scene->addAggregate(*aggregate);

		for (auto piece : pieces)
			PxRigidBodyExt::addForceAtPos(*(PxRigidBody *)piece->physicsBody, (((PxRigidBody *)piece->physicsBody)->getGlobalPose().p - PxVec3(point[0], point[1], point[2])) * damage * 3, PxVec3(point[0], point[1], point[2]), PxForceMode::eIMPULSE);
