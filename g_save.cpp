...
field_t		levelfields[] =
{
	{"changemap", LLOFS(changemap), F_STRING},
...
void WriteField1 (FILE *f, field_t *field, byte *base)
{
	...
	case F_STRING:
		*(int *)p = static_cast<allocated_string *>(p)->length;
		*((int *)p + 1) = static_cast<allocated_string *>(p)->tag;
		break;
	case F_EDICT:
		if ( *(edict_t **)p == NULL)
			index = -1;
...
void WriteField2 (FILE *f, field_t *field, byte *base)
{
	...
	{
	case F_STRING:
		if (static_cast<allocated_string *>(p)->length)
			fwrite (static_cast<allocated_string *>(p)->data, 1, static_cast<allocated_string *>(p)->length, f);
		break;
	}
}
...
void ReadField (FILE *f, field_t *field, byte *base)
{
	...
	case F_STRING:
		...
		if (!len)
			*(allocated_string *)p = allocated_string();
		else
		{
			index = *(((int *)p) + 1);
			*(allocated_string *)p = allocated_string(len + 1, index);
			fread (static_cast<allocated_string *>(p)->data, len, 1, f);
			*(static_cast<allocated_string *>(p)->data + len) = 0;
		}
		break;
	case F_EDICT:
		index = *(int *)p;
